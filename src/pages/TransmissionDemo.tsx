...
const MAFIA_NAMES = [
  "Tony Pepperoni", "Don Margherita", "Garlic Knuckles", "The Mozzarella Kid",
  "Big Cheese Benny", "Vinny No Crust", "Sal Calabrese", "Nina Napoletana",
  "The Sicilian Whisper", "Carmine Crustelli", "Lou Diavola", "Sugo the Silent",
];
...
const TransmissionDemo = () => {
  ...
  useEffect(() => {
    const t = setInterval(() => setMafiaIdx((i) => (i + 1) % MAFIA_NAMES.length), 2200);
    return () => clearInterval(t);
  }, []);

  useEffect(() => {
    const t = setInterval(() => setQuoteIdx((i) => (i + 1) % QUOTES.length), 6500);
    return () => clearInterval(t);
  }, []);
  ...
  return (
    <main className="relative min-h-[100svh] bg-cream text-ink grain">
      ...
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .soft-fade { animation: none !important; } }
      `}</style>
      ...
      <header className="relative z-20">
        <div className="container flex h-16 items-center justify-between md:h-20">
          ...
          <nav className="flex items-center gap-6 md:gap-8">
            <span className="ui rounded-full border border-tomato/40 bg-tomato/10 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-tomato">
              Demo
            </span>
      ...
      <section className="relative z-10">
        <div className="container grid grid-cols-12 gap-x-6 gap-y-12 pb-20 pt-8 md:gap-y-16 md:pb-28 md:pt-12">
          ...
          <div className="col-span-12 md:col-span-4 md:pt-10">
            ...
            <p key={mafiaIdx} className="ui mt-5 text-[12px] text-ink/55 soft-fade">
              Next on deck: <span className="text-ink/85">{MAFIA_NAMES[mafiaIdx]}</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-12 gap-x-6 gap-y-10">
            ...
            <div className="col-span-12 md:col-span-8">
              <div className="grid grid-cols-3 gap-3 md:gap-4">
                {STILLS.map((s, i) => (
                  <figure key={s.src} className={i === 0 ? "col-span-2 row-span-2" : "col-span-1"}>
                    <div className="overflow-hidden rounded-2xl bg-ink/5">
                      <img
                        src={s.src}
                        alt={`PizzaDAO community, ${s.caption}`}
                        loading="lazy"
                        className={`block w-full object-cover transition-transform duration-700 hover:scale-[1.02] ${i === 0 ? "aspect-[4/5]" : "aspect-square"}`}
                      />
                    </div>
      ...
      <section className="relative z-10 bg-butter text-ink">
        <div className="container grid grid-cols-12 gap-x-6 gap-y-8 py-20 md:py-24">
          ...
          <div className="col-span-12 flex items-end md:col-span-5 md:justify-end">
            <a href="https://discord.pizzadao.xyz/" target="_blank" rel="noreferrer" className="btn-pill-lg group bg-ink text-cream hover:bg-tomato">
              Step inside
              <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
            </a>
          </div>
